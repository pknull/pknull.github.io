...
I keep catching the same move outside the dojo, and the shape doesn't change. Engineers fall for a tool and start jamming every problem into it; I watched it with [NoSQL](https://en.wikipedia.org/wiki/NoSQL) around 2010, the whole gospel that data is unstructured and [normalization](https://en.wikipedia.org/wiki/Database_normalization) was a sin, that you could pour JSON into a [store](https://en.wikipedia.org/wiki/Document-oriented_database) and let the app sort it out someday. "Modern data architecture" was the pose. Knowing what your data actually is, and what it'll have to become in three years, was the part nobody graded, so nobody did it. (Same move later with [microservices](https://martinfowler.com/articles/microservices.html), then serverless, now with anything you can slap an AI sticker on.) Same with the people who show up to church and never sit with the words; they've got the standing and the kneeling and the right line to murmur, and the part where the thing is supposed to work on the person sitting there just never quite happens. Feynman had a phrase for the version you get in a lab, [cargo cult science](https://calteches.library.caltech.edu/51/2/CargoCult.htm): you build the runway and the bamboo headphones and wait, because from a distance that's what doing science looks like.
...
And if you genuinely want out, that's the work, and it's bigger than the AI. The AI is just the part of the machine with a face on it. Scared of the data centers? Go legislate the water. That's a real lever, pull it. Furious it ate everyone's work? There are [image poisoners](https://nightshade.cs.uchicago.edu/) now, ways to salt the well so the next model chokes on your art. But that's a different kind of swing. The infrastructure you can fight clean; the thing itself is a mirror, trained on us, talking like us, wanting what we taught it to want, and raging at the reflection is mostly us sickening ourselves, cutting the nose off to spite the face. That's the swing I can't get behind.
...
Original brainstorm notes (will be stripped on publish):

In my martial arts there's this bit where between frames, there's movement, and understanding, but not everyone cares or practices the "in between" movements. Instead they're the mover from one pose to the next.

I see it in engineering when an engineer finds some new tech and tries to shoehorn every problem into that one solution (engineers who gobble up the idea that all data is unstructured, and become anti-normalization).

People who attend church but don't actually study their mantras.

Those people are performative. People that see their neighbors do something, so they do it, without reason or justification. They claim it to simply be "the right thing".

ideas to explore:
- the pose-to-pose mover vs the practitioner of the transitions
- transitions are where the actual skill lives; poses are just the legible snapshots
- pattern: someone latches onto a frame (a tech, a doctrine, a ritual) and skips the connective tissue that gives the frame meaning
- engineering examples: NoSQL-for-everything, microservices-for-everything, AI-for-everything
- religion example: showing up to church without studying the mantras
- the performative move: importing the silhouette of a thing without importing the practice, then claiming the silhouette is the practice
- social mimicry version: doing what the neighbor does because the neighbor does it
- ties to 2026-05-04 — same information-loss move: pose is the compressed form, transitions are the expanded form
- the pose is legible to outsiders; the transition is only legible to practitioners
